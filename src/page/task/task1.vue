  ...
  <div class="task1-page">
    <scroller ref="myscroller" :on-refresh="refresh" :on-infinite="infinite" :loadingLayerColor="bgCol">
      <div class="yichang">
        <!-- 卡片视图 -->
        <ul>
          <li v-for="(item,indexNo) in list" :key="indexNo">
            <div class="card1" v-if="!(item.referId)">
              <div class="titlt">
                ...
                <div class="left">
                  <ul>
                    ...
                    <li>
                      ...
                      <span>任务开始时间:
                        <strong>{{item.deliverPlanDate}}</strong>
                      </span>
                    </li>
                    <li>
              ...
              <router-link :to="{ name: 'task1Detail', params:{taskid: item.id }}" class="chakan">
                  <i></i>
                      <span v-if="item.isDriverConfirmed==null||item.isDriverConfirmed==''">查看详情</span>
                      <span v-if="item.isDriverConfirmed==0"><span class="unconfirmed">[异常签收待确认]</span>查看详情</span>
                      <span v-if="item.isDriverConfirmed==1"><span class="confirmed">[异常签收已确认]</span>查看详情</span>
            ...
            <div class="card" v-else>
                ...
                <div class="barcode">
                    <barcode class="bar" :value="item.referId" :options="{ displayValue: false }"></barcode>
                </div>
                <router-link :to="{ name: 'task1Detail', params:{taskid: item.id }}" class="chakan">
                    <i></i>
                        <span v-if="item.isDriverConfirmed==null||item.isDriverConfirmed==''">查看详情</span>
...
export default {
  ...
  methods: {
    ...
    loadTaskList() {
      ...
      this.axios
        .post(
          `${this.api.loadTaskList}?taskType=1&pageNo=${this.pageNo}&pageSize=${
            this.pageSiz
          }`
        )
        .then(res => {
