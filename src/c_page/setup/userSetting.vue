...
export default {
  ...
  methods: {
    ...
    changimg(file) {
      ...
      this.$router.push("/c_page/userCutImage");
    },
    ...
    imgPreview2(file) {
      ...
      if (/^image/.test(file.type)) {
        ...
        reader.onloadend = function() {
          ...
          self.$router.push('/c_page/userCutImage')
        };
      }
    },
    // 选择用户性别
    ...
    handle(id, res) {
      if (id == 1) {
        ...
        this.$router.push("/c_page/userOperation");
      }
    },
    updateInf(param) {
      this.axios
    ...
    getUserInfos(){
      ...
      this.axios.post(`${this.api.baseURL}/userAuth/getCurrentMobileIdentity`).then(res => {
        ...
        if (res.data.success) {
          ...
          that.user.userSex = res.data.data.result.sex;
          ...
          that.user.isVerified = res.data.data.result.isVerified;
          that.user.userType = 'pcuser';
          ...
          that.checkedboy = res.data.data.result.sex == "1"; // 男默认值
          that.checkedgirl = res.data.data.result.sex == "2"; // 女默认值
        }
      });
    },
    showRealName(){
