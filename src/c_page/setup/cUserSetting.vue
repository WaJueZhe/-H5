  ...
  <div class="set">
      ...
      <div class="form">
          <div v-show="$store.isdemouser" class="photo">
                <span>头像</span>
                <span>
                  <img v-if="user.picUrl != ''&& user.picUrl !=null" v-bind:src="api.resourceurl+user.picUrl">
                  <img v-if="user.picUrl == ''|| user.picUrl ==null" src="static/user.jpg">
          ...
          <div v-show="!$store.isdemouser" class="name" @click="handle(1,user.userName)">
                <span>姓名</span>
                <span style="float:right;">{{user.userName}}<i class="iconfont icon-enter"></i></span>
          </div>
          <div v-show="!$store.isdemouser" class="gender" @click="seletcd=true">
                <span>性别</span>
                <span class="der" v-if="user.userSex==1">男<i class="iconfont icon-enter"></i></span>
                <span class="der" v-if="user.userSex==2">女<i class="iconfont icon-enter"></i></span>
                <span class="der" v-if="user.userSex!=1&&user.userSex!=2">{{user.userSex}}<i class="iconfont icon-enter"></i></span>

          </div>
          <div class="phone">
                <span>手机号码</span>
                <span v-show="!$store.isdemouser">{{user.userMobile}}</span>
                <span v-show="$store.isdemouser">1234567890</span>
          </div>
          <div class="card">   
                <span>身份证号</span>
                <span v-show="!$store.isdemouser">{{user.userIdentity}}</span>
                <span v-show="$store.isdemouser">1234567890</span>
                <span v-show="user.isVerified == '1'" style="float:right;">已认证</span>
                <span v-show="user.isVerified != '1'" style="float:right;color:red" @click="showRealName">未认证</span>
          </div>
      </div>
      ...
      <transition name="selet">
        <div class="leted" v-show="seletcd">
          ...
          <div class="gender">
            <h4>性别</h4>
            <div class="choice">  
                <label class="radio" @click="clkGender(1)">男<input type="radio" name="radio" :checked="checkedbody"><i></i></label>  
      ...
      <transition name="selet">
        <div class="leted" v-show="showReadNameComp">
          ...
          <div class="gender">
            <realName :userName="this.user.userName" :identityCard="this.user.userRealIdentity" :userType="this.user.userType" @completeRealName="completeRealName"></realName>
          </div>
        </div>
      </transition>
...
export default {
  ...
  methods: {
    ...
    changimg(file) {
      ...
      this.$router.push("/c_page/cuserCutImage");
    },
    ...
    imgPreview2(file) {
      ...
      if (/^image/.test(file.type)) {
        ...
        reader.onloadend = function() {
          ...
          self.$router.push('/c_page/cuserCutImage')
        };
      }
    },
    // 选择用户性别
    clkGender(val) {
      ...
      if (this.user.driverSex != dSex) {
        let param = {"userSex": dSex};
    ...
    handle(id, res) {
      if (id == 1) {
        //修改用户姓名
        this.$store.state.pcuser.opertain = "修改姓名";
        this.$store.state.pcuser.behavior = "请输入姓名";
        this.$store.state.pcuser.banggu = res;
        this.$store.state.pcuser.handleId = 1;
        this.$router.push("/c_page/cuserOperation");
      }
    },
    updateInf(param) {
      this.axios
    ...
    getUserInfos(){
      ...
      this.axios.post(`${this.api.baseURL}/userAuth/getCurrentMobileIdentity`).then(res => {
        Indicator.close();
        if (res.data.success) {
          that.user.picUrl = res.data.data.result.picUrl;
          that.user.userName = res.data.data.result.userName;
          that.user.userSex = res.data.data.result.gender;
          that.user.userMobile = res.data.data.result.phone;
          that.user.userIdentity = res.data.data.result.identity;
          that.user.userRealIdentity = res.data.data.result.identity;
          that.user.isVerified = res.data.data.result.isCertfied;
          this.user.userType = 'customer';
          that.user.userMobile = that.user.userMobile.slice(0,3) +'****'+ that.user.userMobile.slice(7);
          if (that.user.userIdentity && (that.user.userIdentity != "")){
            if (that.user.userIdentity.length === 15) {
                  that.user.userIdentity = that.user.userIdentity.slice(0,4) +'****'+'***'+ that.user.userIdentity.slice(11);
            }
            else {
                  that.user.userIdentity = that.user.userIdentity.slice(0,4) +'*****'+'*****'+ that.user.userIdentity.slice(14);
            }
          }
          that.checkedboy = res.data.data.result.gender == "1"; // 男默认值
          that.checkedgirl = res.data.data.result.gender == "2"; // 女默认值
        }
      });
    },
    showRealName(){
