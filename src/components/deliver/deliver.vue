<template>
  <div class="shipment-page">
    <div class="shipment" v-if=" $route.path != '/address' ">装货</div>
    <scroller
      ref="myscroller"
        :on-refresh="refresh"
    ...
    <div class="serch">
      ...
      <div class="group" v-for="item in addressList" :key="item.id">
        <div @click="choseDeliver(item)">
          <div class="code" v-if="identityType == 'pcuser'">编码：{{item.addressCode}}</div>
          <div class="info">
  ...
  methods: {
    ...
    init(){
      ...
      let params = {
          ...
          pageSize: that.limit,
          searchByKeys: {searchStr: that.searchKey,addressType:'D'},
